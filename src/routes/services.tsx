  ...
  head: () => ({
    meta: [
      { title: "Services — DevOps Consulting, Cloud Migration & FinOps | ship.ops" },
      {
        name: "description",
        content:
          "Freelance DevOps consulting, AWS/GCP/Azure cloud migration, Kubernetes platform engineering, and FinOps cost optimization for high-growth teams.",
      },
      { property: "og:title", content: "Services — ship.ops" },
      {
        property: "og:description",
        content:
          "DevOps consulting, cloud migration, and cost optimization engagements. Fixed-scope sprints and retainer options.",
      },
    ],
  }),
...
const packages = [
  {
    ...
    bullets: [
      ...
      "Cloud spend breakdown with quick wins",
      "Security & reliability red flags",
  ...
  },
  {
    ...
    bullets: [
      "Production K8s on AWS/GCP/Azure",
      "GitOps with ArgoCD or Flux",
      "Terraform / Pulumi modules",
      "Observability stack (Prom + Grafana / Datadog)",
      "Runbooks, SLOs, on-call setup",
      "Engineer enablement & handover",
    ],
    cta: "Start a build",
    featured: true,
  },
  {
    ...
    bullets: [
      "Async reviews & architecture calls",
      "Hands-on infra work",
      "Incident response support",
      "Cost reviews each month",
      "Roadmap ownership",
    ],
    cta: "Start retainer",
    featured: false,
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      >
        <div className="grid gap-6 lg:grid-cols-3">
          {packages.map((p) => (
            <div
              key={p.name}
              className={`card-elevated relative flex flex-col rounded-2xl p-8 ${p.featured ? "border-brand/50 glow-ring" : ""}`}
            >
              {p.featured && (
              ...
              <Link
                ...
                className={`mt-8 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition-transform hover:scale-[1.02] ${
                  p.featured
                    ? "bg-gradient-brand text-primary-foreground glow-ring"
      ...
      <Section
        ...
        title={<>From hello to <span className="text-gradient">shipped</span></>}
      >
