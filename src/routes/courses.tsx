  ...
  head: () => ({
    meta: [
      { title: "Cohort Courses — Kubernetes, Terraform, FinOps | ship.ops" },
      {
        name: "description",
        content:
          "Live cohort-based DevOps and cloud engineering courses. Kubernetes in Production, Terraform & IaC Mastery, and Cloud FinOps Bootcamp.",
      },
      { property: "og:title", content: "Cohort Courses — ship.ops" },
      {
        property: "og:description",
        content:
          "6-week live cohorts for engineering teams who want to go from 'we use AWS' to 'we run a real platform'.",
      },
    ],
  }),
...
const courses = [
  {
    ...
    modules: [
      "Cluster anatomy & networking deep-dive",
      "GitOps with ArgoCD",
      "Helm, Kustomize, and progressive delivery",
      "Autoscaling, capacity, and node tuning",
      "RBAC, secrets, and supply chain security",
      "Capstone: ship a real app, end-to-end",
    ],
  },
  {
    ...
    modules: [
      ...
      "State at scale: workspaces, remote backends",
      "Testing with Terratest & checkov",
      "Pipelines, plan reviews, drift detection",
      "Capstone: multi-env infra repo",
    ],
  },
  {
    ...
    modules: [
      ...
      "Architecture-level cost wins",
      "Cost guardrails in CI/CD",
      "Capstone: optimization plan for your account",
    ],
  },
];
      ...
      <section className="relative overflow-hidden pt-20 pb-16">
        ...
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
            Live, cohort-based courses taught by an engineer who runs this stuff in production every week.
            Small groups. Real labs. No fluff.
          </p>
      ...
      <Section className="!pt-12">
        <div className="space-y-6">
          {courses.map((c) => (
            <div
              key={c.name}
              className="card-elevated grid gap-8 rounded-3xl p-8 lg:grid-cols-[1.4fr_1fr] lg:p-12"
            >
              <div>
                {c.tag && (
              ...
              <div className="rounded-2xl border border-border bg-surface/60 p-6">
                ...
                <Link
                  to="/contact"
                  className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-brand px-5 py-3 text-sm font-semibold text-primary-foreground glow-ring transition-transform hover:scale-[1.02]"
                >
                  Enroll team <ArrowRight className="h-4 w-4" />
                </Link>
                <p className="mt-3 text-center text-xs text-muted-foreground">
                  Team discounts for 3+ seats
                </p>
              </div>
            </div>
          ))}
